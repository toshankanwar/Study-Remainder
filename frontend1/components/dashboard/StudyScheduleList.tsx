...
interface StudyScheduleListProps {
  ...
  onToggle: (schedule: StudySchedule) => void;
}
...
export default function StudyScheduleList({ 
  ...
  const handleToggle = async (schedule: StudySchedule) => {
    try {
  ...
  return (
    <div className="space-y-4">
      {schedules.map((schedule) => (
        <div key={schedule._id} className="card hover:shadow-md transition-shadow">
